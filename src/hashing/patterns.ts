...
export const tagTextPattern = (tag: string, text: string, flags: string = 'gim') => new RegExp(`<${tag}[^>]*>[^<]*${escapeRegExpValue(text)}[^<]*<\/`, flags)

export const tagKeyPattern = (tag: string, key: string, flags: string = 'gim') =>
    new RegExp(`<${tag}((?=[^>]+${escapeRegExpValue(key)}(?=\s|>))|(?=[^>]*${escapeRegExpValue(key)}="[^>"]*))*`, flags)
export const keyPattern = (key: string, flags: string = 'gim') => tagKeyPattern('', key, flags)
export const tagKeyValuePattern = (tag: string, key: string, value: string, flags: string = 'gim') =>
    new RegExp(`<${tag}[^>]+${escapeRegExpValue(key)}="${escapeRegExpValue(value)}"[^>]*>`, flags)
export const tagKeyValueLoosePattern = (tag: string, key: string, value: string, flags: string = 'gim') =>
    new RegExp(`<${tag}[^>]+${escapeRegExpValue(key)}="[^>"]*${escapeRegExpValue(value)}[^>"]*"[^>]*>`, flags)
export const tagAttributesPattern = (tag: string, attributes: Record<string, string>, flags: string = 'gim') =>
    new RegExp(
        `<${tag}(${Object.entries(attributes)
            .map(([key, value]) => `(?=[^>]*${escapeRegExpValue(key)}="${escapeRegExpValue(value)}"[^>]*)`)
            .join('|')})*[^>]+>`,
        flags
    )
export const tagAttributesLoosePattern = (tag: string, attributes: Record<string, string>, flags: string = 'gim') =>
    new RegExp(
        `<${tag}[^>]+(${Object.entries(attributes)
            .map(([key, value]) => `(?=[^>]*${escapeRegExpValue(key)}="[^>"]*${escapeRegExpValue(value)}[^>"]*"[^>]*)`)
            .join('|')})*>`,
...
export const metaKeyLooseValuePattern = (key: string, value: string, flags: string = 'gim') => tagKeyValueLoosePattern('meta', key, value, flags)
...
export const linkKeyLooseValuePattern = (key: string, value: string, flags: string = 'gim') => tagKeyValueLoosePattern('link', key, value, flags)
...
export const divKeyLooseValuePattern = (key: string, value: string, flags: string = 'gim') => tagKeyValueLoosePattern('div', key, value, flags)
